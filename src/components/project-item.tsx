...
export default function ProjectItem({
  ...
  return (
    <>
      <motion.div
        className="hidden sm:flex relative group bg-zinc-400/20 hover:bg-zinc-50 items-center transition overflow-hidden"
        variants={parent}
        initial="notHovered"
        whileHover="hovered"
      >
        <div className="w-[120px] md:w-[200px] h-[2px] bg-zinc-400/40" />
      ...
      <a
        href={demoURL}
        target="_blank"
        className="flex sm:hidden h-[160px] group bg-zinc-400/20 items-center pr-4"
      >
        <div className="w-[60px] md:w-[200px] h-[2px] bg-zinc-400/40" />
        <h5 className="ml-2 text-zinc-400 text-lg">0{index}</h5>
        <div className="ml-12 relative w-full flex-1">
          <h3 className="text-zinc-800 font-bold">
            {title}
          </h3>
          <h5 className="text-xs top-6 w-fit text-zinc-600 font-bold">
            {desc}
          </h5>
          <p className="text-xs mt-4">{tech}</p>
        </div>
      </a>
    </>
  );
